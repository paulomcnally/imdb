    ...
    self.getInfo = function( type ){
        ...
        switch ( type ){
            ...
            case 'producers':
                ...
                var producersSerialized = []

                var producers = self.fullCredits$('.dataHeaderWithBorder').eq(3).next().html().match(/<a.*(?=href=\"(?:[^\"]*)\")[^>]*>([^<]*)<\/a>/g)
                producers.forEach(function(producer){

                    producersSerialized.push( S( S(producer).stripTags().s ).trim().s );

                });

                response = producersSerialized || '';
